...
init(Id,  Module, Rnd, Sleep) ->
	%call the start function in the Module with args [Id]
	%Cast should be the id of the start process
	%start a process of Module as a leader
...
init(Id, Module, Rnd, Peer, Sleep) ->
	%to join the peer node
	%cast is the pid of the module process
...
%after wait for some time, there is no message comin, then change the color
    after Wait ->
	    %% Ok, let's propose a change of colors
	    %% io:format("worker ~w mcast message~n", [Id]),
		
		%change is the value predefined
		%send this message to myself for multicasting
